...
public class EPPHostCheckResult implements EPPCodecComponent {
	...
	public Element encode(Document aDocument) throws EPPEncodeException {
		...
		// Host Name
		Element nameElm = aDocument.createElementNS(EPPCodec.NS, ELM_HOST_NAME);
		...
		if (reason != null) {
			Element reasonElm =
				aDocument.createElementNS(EPPCodec.NS, ELM_HOST_REASON);
	...
	public void decode(Element aElement) throws EPPDecodeException {
		// Available
		NodeList elms     = aElement.getElementsByTagName(ELM_HOST_NAME);
		this.available = EPPUtil.decodeBooleanAttr((Element) elms.item(0), ATTR_AVAIL);

		// Name
		name = elms.item(0).getFirstChild().getNodeValue();

		// Host Reason
		NodeList reasonElms = aElement.getElementsByTagName(ELM_HOST_REASON);

		if (reasonElms.getLength() == 1) {
			reason = reasonElms.item(0).getFirstChild().getNodeValue();

			// Language
			String lang =
				((Element) reasonElms.item(0)).getAttribute(ATTR_LANG);
