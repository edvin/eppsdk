...
/**
 * The <code>EPPXMLByteArrayAssembler</code> class provides an implementation
 * of <code>EPPByteArrayAssembler</code> that can assemble/disassemble
...
public class EPPXMLByteArrayAssembler implements EPPByteArrayAssembler {
	...
	private static Logger LOG =
			Logger.getLogger( EPPXMLByteArrayAssembler.class.getName(), EPPCatFactory
	 ...
	 * containing the 17th to 24th bits of the integer and the last array element
	 * containing the last 8 bits of the integer.
	 * 
	 * @param aInteger
	 ...
	 * the "com.verisign.epp.util.EPPXMLParser" as the object type. The remaining
	 * configuration settings are retrieved from the EPPEnv.getServerParser
	 * methods. If there is any error initializing the pool, and error diagnostic
	 * is logged, and the program with stop with a call to
	 * <code>System.exit(1)</code>, since this represents a fatal error. The
	 * <code>EPPEnv</code> settings referenced include:<br>
	 * <br>
	 * <ul>
	...
	public EPPEvent decode ( byte[] aBytes, Object aData )
		...
		try {
			...
			EPPXMLByteArray xmlByteArray =
					new EPPXMLByteArray( EPPSchemaCachingParser.POOL );
	...
	public byte[] encode ( EPPEventResponse aResponse, Object aData )
		...
		try {
			// First, get the message and convert it to a DOM Document using the codec
