...
public class EPPResponse implements EPPMessage {
	...
	public void decode(Element aElement) throws EPPDecodeException, EPPComponentNotFoundException {
		...
		NodeList responseDataElm =
			aElement.getElementsByTagName(ELM_RESPONSE_DATA);

		// Is there any Response Data?
		if (responseDataElm.getLength() == 1) {
			// Decode Concrete Response
			Element responseMap =
				EPPUtil.getFirstElementChild((Element) responseDataElm.item(0));
		...
		// Extension Element
		Element extensionElm = EPPUtil.getElementByTagName(aElement, ELM_EXTENSION);
