...
public class EPPLoginCmd extends EPPCommand {
	...
	protected Element doGenEncode(Document aDocument) throws EPPEncodeException {
		...
		if ((extservices != null) && extservices.elements().hasMoreElements()) {
			...
			Element svcExtension = aDocument.createElement(ELM_EXT_SERVICES);
	...
	protected void doGenDecode(Element aElement) throws EPPDecodeException {
		...
		// Options
		NodeList elms = aElement.getElementsByTagName(ELM_OPTIONS);
		...
		elms = aElement.getElementsByTagName(ELM_SERVICES);
		...
		Element servicesElm = (Element) elms.item(0);

		// Get the list of services
		NodeList serviceElms =
			servicesElm.getElementsByTagNameNS("*", EPPService.ELM_OBJ_URI);
		...
		NodeList svcExtensionNodeList =
			aElement.getElementsByTagNameNS("*", ELM_EXT_SERVICES);
		...
		if (svcExtensionNodeList.getLength() != 0) {
			...
			for (int i = 0; i < svcExtensionNodeList.getLength(); i++) {
				...
				NodeList extensionElms =
					svcExtension.getElementsByTagNameNS("*", ELM_EXT_URI);
