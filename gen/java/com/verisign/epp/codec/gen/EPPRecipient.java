...
public class EPPRecipient implements EPPCodecComponent {
	...
	public void decode(Element aElement) throws EPPDecodeException {
		...
		for (int i = 0; i < theList.getLength(); i++) {
			...
			if (theElm.getNodeName().equals(ELM_OTHER)) {
				this.otherRecipient = true;
			}
			else if (theElm.getNodeName().equals(ELM_OURS)) {
				String theDesc =
					EPPUtil.decodeString(theElm, EPPCodec.NS, ELM_OURS_DESC);
				this.addOurs(theDesc);
			}
			else if (theElm.getNodeName().equals(ELM_PUBLIC)) {
				this.publicRecipient = true;
			}
			else if (theElm.getNodeName().equals(ELM_SAME)) {
				this.sameRecipient = true;
			}
			else if (theElm.getNodeName().equals(ELM_UNRELATED)) {
