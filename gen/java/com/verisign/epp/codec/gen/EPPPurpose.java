...
public class EPPPurpose implements EPPCodecComponent {
	...
	public void decode(Element aElement) throws EPPDecodeException {
		...
		for (int i = 0; i < theList.getLength(); i++) {
			...
			if (theElm.getNodeName().equals(ELM_ADMIN)) {
				this.admin = true;
			}
			else if (theElm.getNodeName().equals(ELM_CONTACT)) {
				this.contact = true;
			}
			else if (theElm.getNodeName().equals(ELM_OTHER)) {
				this.other = true;
			}
			else if (theElm.getNodeName().equals(ELM_PROV)) {
				this.prov = true;
			}
			else {
