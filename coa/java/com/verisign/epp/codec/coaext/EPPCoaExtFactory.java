...
public class EPPCoaExtFactory extends EPPExtFactory {
	...
	public EPPCodecComponent createExtension ( Element aExtensionElm )
		...
		if (aExtensionElm == null) {
			throw new EPPCodecException( "Null extension element." );
		}
		if (aExtensionElm.getTagName() == null) {
			throw new EPPCodecException( "Extension element received with null tag name." );
		}
		String name = aExtensionElm.getTagName();

		if ( name.equals( EPPCoaExtCreate.ELM_NAME ) ) {
			return new EPPCoaExtCreate();
		}
		else if ( name.equals( EPPCoaExtUpdate.ELM_NAME ) ) {
			return new EPPCoaExtUpdate();
		}
		else if ( name.equals( EPPCoaExtInfData.ELM_NAME ) ) {
