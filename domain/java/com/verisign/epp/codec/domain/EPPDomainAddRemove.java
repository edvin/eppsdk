...
public class EPPDomainAddRemove implements EPPCodecComponent {
	...
	public void decode(Element aElement) throws EPPDecodeException {
		if (aElement.getTagName().equals(ELM_ADD)) {
			mode = MODE_ADD;
		}
		else if (aElement.getTagName().equals(ELM_REMOVE)) {
			mode = MODE_REMOVE;
		}
		else if (aElement.getTagName().equals(ELM_CHANGE)) {
			mode = MODE_CHANGE;
		}
		else {
		...
		if (mode == MODE_CHANGE) {
			...
			// Servers
			Element theServersElm =
				EPPUtil.getElementByTagName(aElement, ELM_SERVER);
			...
			if (theServersElm != null) {
				...
				if (theServerElm != null) {
					if (theServerElm.getTagName().equals(ELM_HOST_OBJ)) {
						servers =
					...
					else if (theServerElm.getTagName().equals(ELM_HOST_ATTR)) {
						servers =
