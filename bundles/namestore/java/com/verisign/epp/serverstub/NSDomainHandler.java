...
public class NSDomainHandler extends EPPDomainHandler {
	...
	public NSDomainHandler() {
		...
		try {
			...
			EPPFactory.getInstance().addExtFactory(EPPRgpExtFactory.class.getName());

			cat.info("NSDomainHandler(): Registering EPPSyncExtFactory");
			EPPFactory.getInstance().addExtFactory(EPPSyncExtFactory.class.getName());

		}
		 catch (EPPCodecException ex) {
			cat.error(
					  "NSDomainHandler(): Couldn't load the Map Factory",
					  ex);
			System.exit(1);
		}
	}
	...
	protected void preHandleEvent(EPPEvent aEvent, Object aData)
		...
		if (theMessage.hasExtension(EPPNamestoreExtNamestoreExt.class)) {
			EPPNamestoreExtNamestoreExt theExt = (EPPNamestoreExtNamestoreExt) theMessage.getExtension(EPPNamestoreExtNamestoreExt.class);
			...
			if (theExt.getSubProductID().equals("BAD")) {
				...
				theResponse.addExtension(new EPPNamestoreExtNSExtErrData(
						EPPNamestoreExtNSExtErrData.ERROR_SUB_PRODUCT_NOT_EXISTS));
				throw new EPPHandleEventException("Invalid sub-product id " + theExt.getSubProductID(),
						theResponse);
			}
	...
	protected EPPEventResponse doDomainCreate(EPPEvent aEvent, Object aData) {
		...
		if ( theMessage.hasExtension( EPPCoaExtCreate.class ) ) {
			...
			if ( create != null ) {
				// The requirement that there be one or more attributes is handled by
				// the xsd.
		...
		if (theMessage.getName().equals("NSPollTst.com")) {
			...
			try {
				...
				theTransMsg.setTransferStatus(EPPDomainTransferResp.TRANSFER_PENDING);
				EPPPollQueueMgr.getInstance().put(
												  null, NSPollHandler.KIND,
												  theTransMsg, null);

				// Domain Transfer approve
				theTransMsg.setTransferStatus(EPPDomainTransferResp.TRANSFER_CLIENT_APPROVED);
				EPPPollQueueMgr.getInstance().put(
												  null, NSPollHandler.KIND,
												  theTransMsg, null);

				// Domain Transfer cancelled
				theTransMsg.setTransferStatus(EPPDomainTransferResp.TRANSFER_CLIENT_CANCELLED);
				EPPPollQueueMgr.getInstance().put(
												  null, NSPollHandler.KIND,
												  theTransMsg, null);

				// Domain Transfer rejected
				theTransMsg.setTransferStatus(EPPDomainTransferResp.TRANSFER_CLIENT_REJECTED);
				EPPPollQueueMgr.getInstance().put(
												  null, NSPollHandler.KIND,
												  theTransMsg, null);
				// Domain Transfer approved
				theTransMsg.setTransferStatus(EPPDomainTransferResp.TRANSFER_SERVER_APPROVED);
				EPPPollQueueMgr.getInstance().put(
												  null, NSPollHandler.KIND,
												  theTransMsg, null);
				...
				theTransMsg.setTransferStatus(EPPContactTransferResp.TRANSFER_PENDING);
				EPPPollQueueMgr.getInstance().put(
												  null, NSPollHandler.KIND,
												  theTransMsg, null);

				// Contact Transfer approve
				theTransMsg.setTransferStatus(EPPContactTransferResp.TRANSFER_CLIENT_APPROVED);
				EPPPollQueueMgr.getInstance().put(
												  null, NSPollHandler.KIND,
												  theTransMsg, null);

				// Contact Transfer cancelled
				theTransMsg.setTransferStatus(EPPContactTransferResp.TRANSFER_CLIENT_CANCELLED);
				EPPPollQueueMgr.getInstance().put(
												  null, NSPollHandler.KIND,
												  theTransMsg, null);

				// Contact Transfer rejected
				theTransMsg.setTransferStatus(EPPContactTransferResp.TRANSFER_CLIENT_REJECTED);
				EPPPollQueueMgr.getInstance().put(
												  null, NSPollHandler.KIND,
												  theTransMsg, null);
				// Contact Transfer approved
				theTransMsg.setTransferStatus(EPPContactTransferResp.TRANSFER_SERVER_APPROVED);
				EPPPollQueueMgr.getInstance().put(
												  null, NSPollHandler.KIND,
												  theTransMsg, null);
				...
				theLowBalanceMsg.setCreditThreshold(new EPPLowBalancePollThreshold(EPPLowBalancePollThreshold.PERCENT, "10"));
				...
				theRGPMsg.setReportDueDate(new Date(System.currentTimeMillis() +
		                                                 3600));
				theRGPMsg.setTransId(transId);
				theRGPMsg.setReqDate(new Date(System.currentTimeMillis()));
				theRGPMsg.setStatus(new EPPRgpPollStatus(EPPRgpPollStatus.PENDING_RESTORE));
				EPPPollQueueMgr.getInstance().put(
						  null, NSPollHandler.KIND,
						  theRGPMsg, null);

				// Domain Pending Approved Action Notification
				com.verisign.epp.codec.domain.EPPDomainPendActionMsg thePendActionMsg =
					new com.verisign.epp.codec.domain.EPPDomainPendActionMsg(transId,
							theMessage.getName(), true,
							new EPPTransId(theMessage.getTransId(), svrTransId),
							new Date());
				EPPPollQueueMgr.getInstance().put(
						  null, NSPollHandler.KIND,
	...
	protected EPPEventResponse doDomainUpdate(EPPEvent aEvent, Object aData) {
		...
		cat.debug("doDomainUpdate: commaind = [" + theMessage + "]");
	...
	protected EPPEventResponse doDomainRestoreRequest(EPPEvent aEvent, String aDomainName, EPPTransId aTransId, Object aData) {
		...
		cat.info("domain restore request: domain name = " +
        		aDomainName);
		...
		EPPRgpPollResponse thePollMsg =
            new EPPRgpPollResponse();

        thePollMsg.setName(aDomainName);
        thePollMsg.setReportDueDate(new Date(System.currentTimeMillis() +
                                             3600));
		...
		theResponse.addExtension(
				new EPPRgpExtUpData(
						new EPPRgpExtStatus(EPPRgpExtStatus.PENDING_RESTORE)));

		return new EPPEventResponse(theResponse);
	}

	protected EPPEventResponse doDomainRestoreReport(EPPEvent aEvent, String aDomainName,
			EPPTransId aTransId, EPPRgpExtReport aReport, Object aData) {

        cat.info("domain restore report: domain name = " +
        		aDomainName + ", report = " + aReport);
	...
	protected EPPEventResponse doDomainSync(EPPEvent aEvent, String aDomainName,
			EPPTransId aTransId, EPPSyncExtUpdate aSyncExt, Object aData) {


        cat.info("domain sync: domain name = " +
        		aDomainName + ", month = " +
			aSyncExt.getMonth() + ", day = " +
			aSyncExt.getDay());
