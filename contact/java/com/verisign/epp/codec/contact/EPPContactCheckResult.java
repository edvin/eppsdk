...
public class EPPContactCheckResult implements EPPCodecComponent {
	...
	public Element encode(Document aDocument) throws EPPEncodeException {
		...
		// Contact id
		Element nameElm =
			aDocument.createElementNS(EPPCodec.NS, ELM_CONTACT_ID);
		...
		if (reason != null) {
			Element reasonElm =
				aDocument.createElementNS(EPPCodec.NS, ELM_CONTACT_REASON);
	...
	public void decode(Element aElement) throws EPPDecodeException {
		// Available
		NodeList elms     = aElement.getElementsByTagName(ELM_CONTACT_ID);
		this.available = EPPUtil.decodeBooleanAttr((Element) elms.item(0), ATTR_AVAIL);

		// Id
		id = elms.item(0).getFirstChild().getNodeValue();

		//Contact Reason
		NodeList reasonElms = aElement.getElementsByTagName(ELM_CONTACT_REASON);
